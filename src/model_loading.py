...
def load_movinet_backbone(model_id="a0", resolution=172):
    """ Loades MoViNet backbone using standard setting """
...
def detect_hardware_distribution():
    """ Adpated from TF tutorial for determining running on GPU """
    try:
        tpu_resolver = (
...
def load_seg_model_weights(model_specs, loss_obj=None, optimizer=None):
    """ Loads saved weights given model specs, compiles and returns model"""
    backbone = load_movinet_backbone(
        model_id=model_specs["model_id"], resolution=model_specs["resolution"]
    )
    ...
    with distribution_strategy.scope():
        model = build_classifier(
            ...
            model_specs["num_frames"],
            model_specs["resolution"],
            backbone,
            model_specs["num_classes"],
        )
        if loss_obj is None:
            if (
                (model_specs["num_classes"] <= 2) & (model_specs["loss_name"] is None)
            ) or (model_specs["loss_name"] == "BinaryCrossentropy"):
                loss_obj = BinaryCrossentropy(from_logits=True)
                loss_name = "BinaryCrossentropy"
            elif (
                (model_specs["num_classes"] > 2) & (model_specs["loss_name"] is None)
            ) or (model_specs["loss_name"] == "SparseCategoricalCrossentropy"):
...
def load_seg_model_data(model_path):
    """ Loads data affiliated with a given model folder necessary to format model backbone,
        load weights, and format data """

    files = [x for x in os.listdir(model_path) if "_md_" in x]
    if len(files) > 1:
        target_file = np.argmax(
            [datetime.strptime(x.split("_")[1], "%m%d%y").date() for x in files]
        )
...
def create_seg_model(
    ...
    """ Creates a new instance of a model, using pretrained weights from TF"""
    ...
    with distribution_strategy.scope():
        ...
        if loss_obj is None:
            if ((num_classes == 1) & (loss_name is None)) or (
                loss_name == "BinaryCrossentropy"
            ):
                loss_obj = BinaryCrossentropy(from_logits=True)
                loss_name = "BinaryCrossentropy"
            elif ((num_classes > 1) & (loss_name is None)) or (
                loss_name == "SparseCategoricalCrossentropy"
            ):
                loss_obj = SparseCategoricalCrossentropy(from_logits=True)
                loss_name = "SparseCategoricalCrossentropy"
...
def save_segmentation_model(
    save_path, model_specs, data_specs, accuracy, loss, test_train_split
):
    """ Saves model to folder with necessary model & data specs """
    ...
    model_name = os.path.basename(model_specs["checkpoint_dir"]).split("_checkpoints")[
        0
    ]
    ...
    tr_fn = os.path.join(
        save_path, model_name + "_training_sets", model_name + "_" + today + ".pkl"
    )
...
def view_segmentation_model(model_path):
    """ Easily look at the specs for a given model"""
