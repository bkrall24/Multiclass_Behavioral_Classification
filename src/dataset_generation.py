...
def choose_behavior_subset(folder, classes, splits):
    ...
    for split_name, split_count in splits.items():
        # print(split_name, ":")
        split_files, new_files_for_class = split_class_lists(
            new_files_for_class, split_count
        )
...
def get_files_per_class(folder):
    ...
    Args:
      folder: folder containing subfolders of mp4s.

    Return:
      Dictionary of class names (subfolder names): files
    """
...
def frames_from_video_file(video_path, n_frames, output_size=(172, 172), frame_step=15):
    ...
    Args:
      ...
      output_size: Pixel size of the output frame image.
      frame_step: frequency to pull frames, i.e. 1 == every frame pulled, 2 == every other
...
class FrameGenerator:
    def __init__(self, file_dict, n_frames, frame_step, training=False):
        ...
        Args:
          file_dict: Video file paths with associated labels.
    ...
    def __call__(self):
        ...
        for path, name in pairs:
            video_frames = frames_from_video_file(
                path, self.n_frames, frame_step=self.frame_step
            )
